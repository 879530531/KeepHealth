...
<script type="text/javascript" src="<%=path %>/js/common.js"></script>
    <script type="text/javascript">
        $(function(){
            ...
            if("1" == "${actionState}"){
                alert('更新配方成功！');
                window.location.href="<%=basePath%>/biz/TbFormula_openList.action";
...
<body>
    ...
    <form action="<%= basePath%>/biz/TbFormula_updateFormula.action" method="post" id="commonform">
     
    <ul class="forminfo">
        <li><label>配方内容</label><input name="tbFormula.formulaContext" type="text"  class="dfinput" value="${tbFormula.formulaContext}"/><i><font color="#FF0000">*必填(数字1-8，多个配方请统一以英文逗号或中文逗号分隔)</font>每个配方不能超过30个数字</i></li>
        <li><label>类型</label>
            <select name="tbFormula.formulaType" class="dfinput">
                <option>${tbFormula.formulaType}</option>
       ...
       <%--  <li><label>针对症状详述</label><textarea name="tbFormula.formulaSymptomDetails"  class="dfinput"  cols="5" rows="5">${tbFormula.formulaSymptomDetails}</textarea><i><font color="#FF0000">*</font>不得超过50个汉字</i></li>--%>
        <input name="tbFormula.formulaState" value="${tbFormula.formulaState}" type="hidden">
        <input name="tbFormula.formulaId" value="${tbFormula.formulaId}" type="hidden">
        <li><label>&nbsp;</label><input name="add_btn" type="submit" class="btn" value="确认修改"/>
