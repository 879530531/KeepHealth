...
<script type="text/javascript">
	...
	$(document).ready(function(){
			...
			var url;
 			...
 			url =" <%= basePath%>/biz/Content_delete.action?id="+id;
 			 $(".tip").fadeIn(200);
 			 });
...
<%int i=1; %>
    ...
    <div id="usual1" class="usual">
      <div id="tab2" class="tabson">
        <form action="<%= basePath%>/biz/Content_list.action" method="post"  target="rightFrame">
    	<ul class="seachform">
            ...
            <li><input type="submit" class="scbtn" value="查询"/></li>
           	 	<li class="clickk"><span><img src="<%=path%>/images/t01.png" /></span><a href="${path}/biz/Content_openAdd.action" >发布学习资料</a></li>
        </ul>
        </form> 
    ...
    <table class="tablelist">
       ...
       <s:iterator value="pageResult.data" var="i">
        <tr>
        <td><%=i++ %></td>
        <td><a href="<%= basePath%>/biz/ContentWeiXin_showWeb.action?id=${i.id}" ><font color="#056dae">${i.title}</font></a></td>
        <td>${i.detail}</td>
        <td>${map[i.userId].username}</td>
        <td>${i.operateTime}</td>
        <td>${i.type}</td>
        <td>
        		<!--  <a href="<%= basePath%>/biz/ContentWeiXin_showWeb.action?id=${i.id}" class="tablelink">查看</a>&nbsp;&nbsp; -->
	     	 	<a href="javascript:;" class="tablelinkdelete" id_value="${i.id}" > 删除</a>&nbsp;&nbsp; 
	        	<a href="<%= basePath%>/biz/Content_openEdit.action?id=${i.id}" class="tablelink">修改</a> 
        </td>
...
<%
//查询的url地址，统一写
String listActionURL = basePath+"/biz/Content_list.action";
%>
...
<script type="text/javascript">
var url = "<%= basePath%>/biz/Content_list.action";                 //获取表单url
//首页
function first(){
