...
<div class="formbody">
    <div id="usual1" class="usual">
      <div id="tab2" class="tabson">
     	...
     	<form action="<%= basePath%>/biz/TbPersonalData_list.action?flagType=1" method="post"  target="rightFrame">
    	<ul class="seachform">
    	    <li><label>导师编号</label><input class="scinput" name="tbPersonalData.personalNumber"   placeholder="请输入导师编号"/></li>
    	    <li><label>导师姓名</label><input class="scinput" name="tbPersonalData.realName"   placeholder="请输入导师名称"/></li>
            <li><input name="" type="submit" class="scbtn" value="查询"/></li>
            <p:permissions right="addTeacher" >
            	<li class="clickk"><span><img src="<%=path%>/images/t01.png" /></span><a href="<%= basePath%>/biz/SysTest_openAdd1.action">添加导师</a></li>
            </p:permissions>
        </ul>
        </form> 
    <table class="tablelist">
        ...
        <th>指导方向</th>
        <th>导师简介</th>
        <p:permissions menu="deleteTeacher,editTeacher,editPassword">
        ...
        <td>${direction}</td>
        <td style="width:23%" title="${introduction}"><div class="textoverr">${introduction}</div></td>
        <p:permissions menu="deleteTeacher,editTeacher,editPassword">
        <td style="width:12%">
	     	<p:permissions right="deleteTeacher">
	     		<a href="javascript:;" class="tablelinkdelete" id_value = "${personalId}"  value = "${flagType}" > 删除</a>&nbsp;&nbsp;
	     	</p:permissions>
	     	
        	<p:permissions right="editTeacher" >
...
function change()
  {
 	...
 	if(totalPage*1 >= textfield*1 ){
 		pageNum = textfield; 
 		window.location.href="<%=listActionURL%>?page="+pageNum+"&flagType=1";
 	}else{
 		pageNum = totalPage; 
...
<script type="text/javascript">
var url = "<%= basePath%>/biz/TbPersonalData_list.action?flagType=1";                 //获取表单url
//首页
function first(){
