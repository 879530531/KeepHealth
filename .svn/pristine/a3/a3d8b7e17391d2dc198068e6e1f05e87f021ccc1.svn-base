...
<body>
    ...
    <div class="formtitle"><span>导师信息</span></div>
   <%--会员编号和联系方式不用改，但要输出值，value的值除了密码也要输出 --%>
    <table> 
     <tr><td align='right' height="30"> 导师编号：</td><td> ${tbPersonalData.personalNumber}</td></tr> 
    ...
    <tr><td align='right' height="30">导师级别：</td><td> 
        <c:choose>
	        ...
	        <c:when test="${2 == tbPersonalData.level}">
	        	高级导师
	        </c:when>
	        <c:otherwise>
    ...
    <tr><td align='right' height="30">指导方向：</td><td>${tbPersonalData.direction}</td></tr> 
    <tr><td align='right' height="30">导师简介：</td><td><span style="width:300px;text-align: justify;padding-bottom:3px">${tbPersonalData.introduction}</span></td></tr> 
    <tr><td align='right' height="30" valign="top">头像：</td><td><img src="/upload/${tbPersonalData.image}"  alt="${tbPersonalData.image}" width="150" /></td></tr> 
